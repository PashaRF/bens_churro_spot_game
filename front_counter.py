...
class FrontCounter:
    ...
    def check_for_new_customers(self, game_state):
        """Checks if the bell rang (someone is in waiting_at_door) and takes their order."""
        print("\n[Front Counter] Checking the register...")

        # --- THE FIX: Look at the dedicated waiting_at_door list ---
        if not game_state.get('waiting_at_door'):
            print("No one is at the counter right now. Listen for the bell!")
            return None

        # A customer is waiting! Pop the first person in line
        pending_customer = game_state['waiting_at_door'].pop(0)
        ...
        # Put the ACTUAL Ticket object into game_state['tickets']
