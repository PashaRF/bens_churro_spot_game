...
def handle_input(choice, game_state):
    ...
    if choice == "4":
        ...
        # Pass game_state so the counter can check for pending customers from the bell
        new_ticket = counter.check_for_new_customers(game_state)
        return new_ticket
...
class FrontCounter:
    ...
    def check_for_new_customers(self, game_state):
        """Checks if the bell rang (a dict is in tickets) and takes their order."""
        print("\n[Front Counter] Checking the register...")

        # 1. Look for a pending customer (a dictionary) in the tickets list
        pending_index = -1
        if 'tickets' in game_state:
            for i, ticket_data in enumerate(game_state['tickets']):
                if isinstance(ticket_data, dict):  # Real orders are Ticket objects
                    pending_index = i
                    break

        # 2. If no dictionary is found, no one is waiting!
        if pending_index == -1:
            print("No one is at the counter right now. Listen for the bell!")
            return None

        # 3. A customer is waiting! Remove their placeholder dict from the list
        pending_customer = game_state['tickets'].pop(pending_index)

        # We can use the name generated by main, or pick a new one from NAMES_POOL
        customer_name = pending_customer.get('name', random.choice(NAMES_POOL))
        new_customer = Customer(customer_name)

        # 4. Generate random ticket details
        ...
        # Put the ACTUAL Ticket object back into game_state['tickets']
        game_state['tickets'].append(new_ticket)

        print(
            f"--> {new_customer.name} stepped up to the counter and placed an order!")
        print(new_ticket)
