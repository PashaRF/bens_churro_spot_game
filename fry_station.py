...
def handle_input(choice, game_state):
    ...
    if choice == "4":
        ...
        chosen_shape = input(
            "Type dough shape to load (or press enter to cancel): ").strip().title()
        ...
        if chosen_shape in game_state["pastry_to_fry_queue"]:
            try:
                ...
                if f_num in station.fryers and station.fryers[f_num].count == 0:
                    ...
                    if qty in [1, 2]:
                        available_count = game_state["pastry_to_fry_queue"].count(
                            chosen_shape)
                        if available_count >= qty:
                            # Remove dough from the pastry queue
                            for _ in range(qty):
                                game_state["pastry_to_fry_queue"].remove(
                                    chosen_shape)
                            # Load the fryer
                            station.fryers[f_num].add_churros(
                                chosen_shape, qty)
                            print(
                                f"\nDropped {qty} {chosen_shape} churro(s) into Fryer #{f_num}!")
                        else:
                            print(
                                f"You don't have {qty} units of {chosen_shape} ready.")
                    else:
        ...
        try:
            ...
            if f_num in station.fryers:
                ...
                if fryer.count > 0:
                    ...
                    print(
                        f"Final Cook Profile -> Side 1: {side1}s, Side 2: {side2}s")
                    fryer.clear()
                else:
...
class FryStation:
    ...
    def display_all_fryers(self):
        """Displays the capacity status of all 8 fryers (Reverted to Original layout)."""
        print("\n--- Fryer Statuses ---")
        for i in range(1, 9):
            fryer = self.fryers[i]
    ...
    def display_specific_fryer(self, fryer_num: int):
        """Displays detailed cooking information targeted strictly at the active side."""
        ...
        # Determine current active side tracking context
        current_side_time = side2 if fryer.is_flipped else side1
        active_side_label = "Side 2" if fryer.is_flipped else "Side 1"

        # Trigger carriage return error string if overcooked
        if current_side_time > COOK_TIME_PER_SIDE:
            print(
                f"\r[🚨 CRITICAL WARNING: Fryer #{fryer_num} has EXCEEDED optimal cook limits on {active_side_label}!]")

        print(f"\n--- Detailed Status: Fryer #{fryer_num} ---")
        print(f"Churro Type: {fryer.shape}")
        print(f"Quantity: {fryer.count}")

        # Emphasize current active side timer values
        print(
            f"Current Side ({active_side_label}) Cooked: {current_side_time}s / {COOK_TIME_PER_SIDE}s")
        print(f"Completed Side Cooked: {side1 if fryer.is_flipped else 0.0}s")
